...
class TrajectoryTransformer:
    ...
    # def generate_exponential_trajectory(self, base_scaling=1.0, exponent_scaling=1.0):
    #     t = np.arange(0 + self.dt, 30, self.dt)
    #     x = t
    #     y = base_scaling * np.power(np.e, -x/exponent_scaling, )
    #     s = 0.5 * (np.log(np.sqrt(np.power(np.e, 2*x) + 1) - 1) - np.log(np.sqrt(np.power(np.e, 2*x) + 1) + 1)) + np.sqrt(np.power(np.e, 2 * x)) - np.sqrt(2)
    #     n = 1 / np.power(np.e, x)
    #     for i in range(1, len(s)):
    #         s[i] = s[i] + s[i-1]
    #     return Path(x, y, s, n)
    
    # def generate_circular_obstacles(self, x_c, y_c, radius):
    #     """
    #     (x_c, y_c), r -> center and radius
    #     x = x_c + r*cos(theta)
    #     y = y_c + r*sin(theta)
    #     """
    #     t = np.arange(0, 2 * np.pi + self.dt, self.dt)
    #     x = x_c + radius * np.cos(t)
    #     y = y_c + radius * np.sin(t)
    #     return (x , y)
...
class RobotSimulator:
    ...
    def __init__(self, dt=0.1, a=5.0):
        ...
        # Cost function
        deviation_cost = 3 * state_d ** 2
        progress_cost = - 50 * state_s
        ...
        # Obstacle -> Think penalizing d more than s?
    ...
    def simulate(self, n_steps):
        ...
        x0 = np.array([0.1, 0.1, 0.0, 0.0]) # Initial condition in s and d
